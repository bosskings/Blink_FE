    ...
    >
      <View className="relative">
        {item.tag === "SALE" && (
          <View className="absolute top-3 left-3 border border-blue-500 bg-[#0066CC] px-3 py-2 rounded-full z-10">
            <Text style={{}} className="text-white text-xs font-bold">
              {item.tag}
            </Text>
          </View>
        )}
      ...
      <View className="p-4">
        <View className="flex-row justify-between items-start mb-2">
          ...
          <Text style={{}} className="text-[#0066CC] font-bold text-lg">
            {item.price}
          </Text>
        </View>
        <Text style={{}} className="text-gray-600 text-sm mb-3">
          {item.description}
        </Text>
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center">
            <Ionicons name="location-outline" size={14} color="#6B7280" />
            <Text style={{}} className="text-xs text-gray-500 ml-1 mr-3">
              {item.distance}
            </Text>
          </View>
          <View className="flex-row items-cente">
            <Ionicons name="time-outline" size={14} color="#6B7280" />
            <Text style={{}} className="text-xs text-gray-500 ml-1">
              {item.timePosted}
            </Text>
          </View>
        </View>
